...
# def human_clarify_prompt(state: StoryboardState):
#     # Not implemented
#     pass
#     return state
...
class NarrativeClass(BaseModel):
    narrative: str

def human_clarify_prompt(state: StoryboardState):
    # Load XML instructions
    instructions = load_xml_instructions("sbgenerator/human_clarify_prompt.xml")
    ...
    if parsed_output is None:
        raise ValueError("LLM output could not be parsed. Check the response format and prompt instructions. This is node human_clarify_prompt. " + output)

    return { 'narrative': output['parsed'].narrative, 'narrative_modif': '', 'isaccurate': False }
